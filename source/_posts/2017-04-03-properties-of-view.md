...
setWidth(int) 属性说明: 设置文本区域的宽度，支持度量单位：px(像素)/dp/sp/in/mm(毫米)。
 ...
 ## update
View.getScrollX和getScrollY返回的是mScrollX和mScrollY。这个值在View.draw()方法中用到了:
```java
if (offsetForScroll) {
...
```
所以只是一个canvas的translate罢了。

另外，在draw里面还有一段直接buildCache的方法.
View.draw()
```java
Bitmap cache = null;
      ...
      if (drawingWithRenderNode) {
          ...
          ((DisplayListCanvas) canvas).drawRenderNode(renderNode); //走RenderNode
      } else {
