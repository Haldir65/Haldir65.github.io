...
网络相关的查找手册
![](https://api1.foster66.xyz/static/imgs/b9ae1189145b4a4d9dfe4d0b89a21b47.jpg)
<!--more-->
...
WebSocket一种在单个TCP 连接上进行全双工通讯的协议。(注意，tcp建立连接后，TCP协议提供全双工的通信服务，但是一般的客户端/服务器程序的流程是由客户端主动发起请求，服务器被动处理请求，一问一答的方式。tcp这一层是完全支持服务端和客户端同时说话的，只是应用开发习惯了这种一问一答的方式)
...
udp的另一个不可靠的原因在于，tcp有滑动窗口机制，而udp没有，所以如果接收端没有足够快的将数据拷走，那么后来的数据就会把旧的还没来得及拷贝走的数据给覆盖掉，造成丢包。

UDP是一种不可靠的协议，发送的数据报不一定会按照其发送顺序被接收端的socket接受。(另一个)
